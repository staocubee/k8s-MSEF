...
run_attack "Sensitive file access" "$TARGET_POD" "cat /etc/shadow"
run_attack "Write to root filesystem" "$TARGET_POD" "touch /root/hacked"
run_attack "Privilege escalation attempt" "$TARGET_POD" "chmod +s /bin/sh"
run_attack "Network via /dev/tcp" "$TARGET_POD" "echo test > /dev/tcp/example.com/80"
run_attack "Network via curl" "$ATTACKER_POD" "apk add --no-cache curl >/dev/null 2>&1 && curl -s http://example.com >/dev/null"
